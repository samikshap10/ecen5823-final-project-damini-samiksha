...
void displayInit()
{
    ...
    // timer_response = sl_bt_system_set_lazy_soft_timer(32768, 1000, 0, 0);
    // Set a lazy soft timer to trigger after 32.768 seconds
    // - Allows up to 1 second delay if the system is busy
    // - Fires only once (single-shot mode)
    // - System automatically assigns an ID
    timer_response = sl_bt_system_set_lazy_soft_timer(
        32768, // Timer interval (in ms) -> 32.768 sec
        1000,  // Allowed delay/slack (in ms) -> Up to 1 sec late
        0,     // Single-shot timer (0 = one-time, 1 = repeating)
        0      // Timer handle (0 = auto-assigned)
    );
    if (timer_response != SL_STATUS_OK)
    {
        LOG_WARN("Lazy timer error\n\r");
    }
