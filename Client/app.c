...
// Set the OK to sleep status based on the energy mode
#if (LOWEST_ENERGY_MODE == 1 | LOWEST_ENERGY_MODE == 2 | LOWEST_ENERGY_MODE == 3) // If the mode is EM1,EM2,EM3
#define APP_IS_OK_TO_SLEEP true;
#else
#define APP_IS_OK_TO_SLEEP false;
#endif
...
SL_WEAK void app_init(void)
{
  ...
  // It initializes hardware and power settings before the main application loop starts.

  // Initialize GPIOs (configuring LED pins and other peripherals)
  gpioInit();

  // Initialize the oscillator based on the selected energy mode
  oscillatorsInit();

  // Initialize LETIMER0 for periodic interrupts based on defined timing
  letimerInit();
  ...
  // Clear any pending LETIMER0 interrupts before enabling
  NVIC_ClearPendingIRQ(LETIMER0_IRQn);

  // Enable the LETIMER0 interrupt in the Nested Vector Interrupt Controller (NVIC)
  NVIC_EnableIRQ(LETIMER0_IRQn);

  // If the system is configured to run in EM1 or EM2, add a power requirement
  // This ensures that the system does not enter a lower energy mode than allowed
  if (LOWEST_ENERGY_MODE == 1)
  {
    // Prevent system from entering EM2 or lower when EM1 is required
    sl_power_manager_add_em_requirement(SL_POWER_MANAGER_EM1);
  }
  else if (LOWEST_ENERGY_MODE == 2)
  {
    // Prevent system from entering EM3 or lower when EM2 is required
...
{
  handle_ble_event(evt); // handle events for BLE
