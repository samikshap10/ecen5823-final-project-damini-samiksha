...
void displayInit()
{
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  timer_response = sl_bt_system_set_lazy_soft_timer(TIMER_INTERVAL_1HZ,//01
	                                                    TIMER_SLACK, //500
	                                                    TIMER_HANDLE_1HZ,//32768
	                                                    0); // 0 for repeating timer
	  if (timer_response != SL_STATUS_OK) {
	      LOG_ERROR("Bluetooth: Set lazy soft timer = %d\r\n", (unsigned int) timer_response);
     }
...
/**
 * Call this function from your event handler in response to
