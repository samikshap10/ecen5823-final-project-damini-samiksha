    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      ...
      if((evt->data.evt_gatt_server_characteristic_status.characteristic == gattdb_temperature_measurement)
          ...
          if(dequeue(&indicationQueue, &dequeuedIndication)){
              if(dequeuedIndication.charHandle == gattdb_temperature_measurement){
      ...
      if((evt->data.evt_gatt_server_characteristic_status.characteristic == gattdb_button_state)
          ...
          if(dequeue(&indicationQueue, &dequeuedIndication)){
              if(dequeuedIndication.charHandle == gattdb_button_state){
                  // Send the indication to the client
                  rc = sl_bt_gatt_server_send_indication(
                      ble_data.connectionHandle,
    ...
    case sl_bt_evt_system_external_signal_id:
      if (evt->data.evt_system_external_signal.extsignals == EVENT_PB0) {
          ...
          if(ble_data.expecting_passkey_confirmation == true){
              ...
              uint8_t button_state = (GPIO_PinInGet(PB0_PORT, PB0_PIN) == 0) ? BUTTON_PRESSED : BUTTON_RELEASED;
              rc = sl_bt_gatt_server_write_attribute_value(gattdb_button_state, 0, sizeof(uint8_t), &button_state);
              if (rc == SL_STATUS_OK && ble_data.bonded && ble_data.ok_to_send_bs_connections && ble_data.connection_open) {
                  send_next_indication(button_state);
              }
              displayPrintf(DISPLAY_ROW_9, button_state ? "Button Pressed" : "Button Released");
          }
...
void send_next_indication(uint8_t button_state) {
  ...
  newIndication.charHandle = gattdb_button_state;
  newIndication.bufferLength = 1;
  newIndication.buffer[0] = button_state;
