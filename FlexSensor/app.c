...
//           think "#if (expression)".
//set APP_IS_OK_TO_SLEEP to false for EM0, and to true for EM1, EM2 and EM3
...
SL_WEAK void app_init(void)
{
  ...
  CMU_init(); // Initialize Oscillator and Clock
  gpioInit(); // Initialize LED0 and LED1
  LETIMER0Init(); // Configure LETIMER
  LETIMER0EnableIrq(); // Enable Interrupt for LETIMER
  I2C0_init(); // Configure I2C0
  initADC();
...
SL_WEAK void app_process_action(void)
{
 ...
 // evt = getNextEvent();//gets the event
 // stateMachineTemeratureRead(evt);
// if (evt == EVENT_PB0){
 //    gpioLed0Toggle();
 //    timerWaitUs_irq(500000);//Normal case
 //}
  //timerWaitUs_irq_UnitTest(evt);
  //timerWaitUs_poll_UnitTest();
...
#else
   stateMachineTemperatureRead(evt);
#endif
